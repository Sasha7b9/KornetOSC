...
void FrequencyCounter::SetStateLampFreq(uint16 flag)
{
    if(!lampFreq)
    {
        if(_GET_BIT(flag, FL_TRIG_READY) == 0)      // ≈сли пришЄл первый импульс счЄта
        {
            if(_GET_BIT(flag, FL_FREQ_READY) == 0)  // » не готов счЄтчик частоты
            {
                lampFreq = true;
            }
        }
...
void FrequencyCounter::SetStateLampPeriod(uint16 flag)
{
    if (!lampPeriod)
    {
        if (_GET_BIT(flag, FL_TRIG_READY) == 0)      // ≈сли пришЄл первый импульс счЄта
        {
            if (_GET_BIT(flag, FL_PERIOD_READY) == 0)  // » не готов счЄтчик частоты
            {
                lampPeriod = true;
            }
        }
