...
void Menu::ProcessKeyEvent(KeyEvent event)
{
    static const pFuncVKE func[Key::NumButtons][4] =
    {
        ...
        /* ChannelA    */ {Handlers::Chan,        Handlers::Chan,        Handlers::Chan,     Handlers::Chan},
        /* ChannelB    */ {Handlers::Chan,        Handlers::Chan,        Handlers::Chan,     Handlers::Chan},
        ...
        /* RangeMoreA  */ {Handlers::Range,       Handlers::Range,       Handlers::Range,    Handlers::Range},
        /* RangeLessA  */ {Handlers::Range,       Handlers::Range,       Handlers::Range,    Handlers::Range},
        ...
        /* RangeMoreB  */ {Handlers::Range,       Handlers::Range,       Handlers::Range,    Handlers::Range},
        /* RangeLessB  */ {Handlers::Range,       Handlers::Range,       Handlers::Range,    Handlers::Range},
        ...
        /* TBaseMore   */ {Handlers::TBase,       Handlers::TBase,       Handlers::TBase,    Handlers::TBase},
        /* TBaseLess   */ {Handlers::TBase,       Handlers::TBase,       Handlers::TBase,    Handlers::TBase},
        /* TShiftMore  */ {Handlers::TShift,      Handlers::TShift,      Handlers::TShift,   Handlers::TShift},
        /* TShiftLess  */ {Handlers::TShift,      Handlers::TShift,      Handlers::TShift,   Handlers::TShift},
        /* TrigLevMore */ {Handlers::TrigLev,     Handlers::TrigLev,     Handlers::TrigLev,  Handlers::TrigLev},
        /* TrigLevLess */ {Handlers::TrigLev,     Handlers::TrigLev,     Handlers::TrigLev,  Handlers::TrigLev},
...
void Menu::ShortPressureButton(Key button)
{
    if (!HINT_MODE_ENABLED)
    {
        if (button.Is(Key::Memory) && FDrive::IsConnected() && MODE_BTN_MEMORY_IS_SAVE)
        {
...
void Menu::ProcessButtonForHint(Key button)
{
    ...
    else if (button.Is(Key::Measures))
    {
        Menu::stringForHint = LANG_RU ?
...
{
    if(!shortPressureButton.Is(Key::None))
    {
        ...
        do
        {
            if(button.Is(Key::Enter))                                   // Если нажата кнопка МЕНЮ и мы не находимся в режме настройки измерений.
            {
                if(!MENU_IS_SHOWN)
                {
                    Menu::Show(true);
            ...
            else if (MENU_IS_SHOWN && Keyboard::IsFunctionalButton(button))       // Если меню показано и нажата функциональная клавиша
            {
                void *item = itemUnderButton[button.code];
                if (HINT_MODE_ENABLED)
                {
                    SetItemForHint(item);
                }
...
void Menu::ProcessingLongPressureButton()
{
    ...
    if(!button.Is(Key::None))
    {
        ...
        {
            item = (Control *)itemUnderButton[button.code];
            if(item)
            {
                item->LongPress();
            }
...
void Menu::ProcessingPressButton()
{
    if ((pressButton.code >= Key::F1 && pressButton.code <= Key::F5) || pressButton.Is(Key::Enter))
    {
        ...
        {
            itemUnderKey = itemUnderButton[pressButton.code];
        }
    }
...
{
    if((releaseButton.code >= Key::F1 && releaseButton.code <= Key::F5) || pressButton.Is(Key::Enter))
    {
        itemUnderKey = 0;
...
{
    for (int i = 0; i < Key::NumButtons; i++)
    {
        itemUnderButton[i] = 0;
    }
}
...
{
    if (Menu::IsMinimize() && button.code >= Key::Enter && button.code <= Key::F5)
    {
        Page *page = (Page *)OpenedItem();
        SButton *sb = (SButton *)page->items[button.code - Key::Enter];
        return sb;
    }
    return NULL;
}
